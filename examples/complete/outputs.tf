...
output "instance_lifecycle" {
  value       = length(var.spot_options_attributes[*]) != 0 ? module.ec2_instance.instance_lifecycle : null
  description = "Indicates whether this is a Spot Instance or a Scheduled Instance"
}

output "spot_instance_request_id" {
  value       = length(var.spot_options_attributes[*]) != 0 && var.market_type == "spot" ? module.ec2_instance.spot_instance_request_id : null
